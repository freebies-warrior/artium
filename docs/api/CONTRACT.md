...
- The AI worker should download images using `image_get_urls`, then call the backend's `/items/{item_id}/features` endpoint with its configured backend base URL once features are extracted.
