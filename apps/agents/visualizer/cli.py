...
def main() -> None:
    arguments = argparse.ArgumentParser(
        description="Install artwork into a room (Gemini-only)."
    )
    arguments.add_argument(
        "--room", required=True, help="Path to room image (jpg/png)."
    )
    arguments.add_argument(
        "--art", required=True, help="Path to artwork image (jpg/png)."
    )
    arguments.add_argument(
        "--out", required=True, help="Path to output image (png recommended)."
    )

    arguments.add_argument(
        "--max-retries", type=int, default=None, help="Override retry count."
    )
    arguments.add_argument(
        "--image-model", type=str, default=None, help="Override Gemini image model."
    )
    arguments.add_argument(
        "--text-model", type=str, default=None, help="Override Gemini text model."
    )
    arguments.add_argument(
        "--no-enhance", action="store_true", help="Disable room enhancement step."
    )
