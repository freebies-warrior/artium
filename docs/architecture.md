...
flowchart LR
  ...
  A --> DB[(Postgres)]
  A --> S[(Cloudflare R2 Object Storage)]

  subgraph AI Agents
    V[Visualizer]
    P[Valuation / Feature Extraction]
  end
...
flowchart LR
  ...
  BE -->|HTTP: trigger job| AIx[AI Feature Extraction + Valuation]

  AIv -->|GET inputs / PUT outputs| R2
  AIx -->|GET inputs| R2

  AIv -->|Callback update| BE
  AIx -->|Callback update| BE
```
...
sequenceDiagram
  ...
  FE->>BE: POST /visualizations (item_id, item_image_key, room_image_key)
...
### Diagram: Publish new auction item + feature extraction
```mermaid
sequenceDiagram
...
### Diagram: Feature extraction + valuation workflow (async job with retries)
```mermaid
flowchart TD
  Start([Item created]) --> Presign[Backend presigns GET URLs]
  Presign --> Trigger[Trigger feature extraction job]
  Trigger --> Extract[AI extracts features, including valuation, JSON]
  Extract --> CallbackF[Callback /items/:id/features]
  CallbackF --> StoreV[Backend stores items.features]
